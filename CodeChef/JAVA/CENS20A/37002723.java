///// https://www.codechef.com/viewsolution/32400149 /////
...
class Solver {
    ...
    void solve() throws Exception {
        ...
        for (int tce = 1; tce <= tc; ++tce) {
            ...
            int Q = hp.nextInt();
            for (i = 0; i < Q; ++i) {
                int x1 = hp.nextInt() - 1, y1 = hp.nextInt() - 1;
                int x2 = hp.nextInt() - 1, y2 = hp.nextInt() - 1;

                A[x1][y1] += 1;
                if (y2 + 1 < M) A[x1][y2 + 1] -= 1;
                if (x2 + 1 < N) A[x2 + 1][y1] -= 1;
                if (y2 + 1 < M && x2 + 1 < N) A[x2 + 1][y2 + 1] += 1;
            }

            for (i = 0; i < N; ++i) for (j = 1; j < M; ++j) A[i][j] += A[i][j - 1];
            for (i = 1; i < N; ++i) for (j = 0; j < M; ++j) A[i][j] += A[i - 1][j];
            for (i = 0; i < N; ++i) for (j = 0; j < M; ++j) A[i][j] += matrix[i][j];
