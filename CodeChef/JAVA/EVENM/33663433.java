...
class Solver {
    ...
    public boolean isValid(int[][] mat, int n){
        for(int r = 0; r < n; r++) for(int c = 0; c < n; c++) for(int a = 0; a < n - Math.max(r, c); a++){
            if((mat[r][c] + mat[r+a][c+a]) % 2 != 0) return false;
            if((mat[r+a][c] + mat[r][c+a]) % 2 != 0) return false;
        }
